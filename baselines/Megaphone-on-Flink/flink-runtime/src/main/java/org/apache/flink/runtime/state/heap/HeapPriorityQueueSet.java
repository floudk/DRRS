...
//        return keyGroup - keyGroupRange.getStartKeyGroup();
    ...
    @Nonnull
    @Override
    public Map<Integer,Set<T>> getAndRemoveSubsetForKeyGroup(List<Integer> keyGroups) {
        Map<Integer,Set<T>> subset = new HashMap<>();

        List<Integer> localIndex = new ArrayList<>();
        keyGroups.forEach(keyGroup ->{
            localIndex.add(globalKeyGroupToLocalIndex(keyGroup));
            HashMap<T, T> dedupMap = getDedupMapForKeyGroup(keyGroup);
            Set<T> sub = dedupMap.keySet();
            for (T element : sub) {
                super.remove(element);
            }
            dedupMap.clear();
            subset.put(keyGroup, sub);
        });

        // sort the localIndex to remove from the end
        localIndex.sort((o1, o2) -> o2 - o1);
        for (int index : localIndex) {
            deduplicationMapsByKeyGroup.remove(index);
        }

        LOG.info("Remove keyGroups {} from deduplicationMapsByKeyGroup, left keyGroupRange {}({}), deduplicationMapsByKeyGroup length: {}",
                keyGroups,
                keyGroupRange,
    ...
    public void mergeKeyGroup(int mergedKeyGroup){
        if (keyGroupRange.getNumberOfKeyGroups() <= deduplicationMapsByKeyGroup.size()){
            LOG.info("Skip merge keyGroupID {} to deduplicationMapsByKeyGroup, current keyGroupRange {}({}) with deduplicationMapsByKeyGroup length: {}",
                    mergedKeyGroup,
                    keyGroupRange,
                    keyGroupRange.getNumberOfKeyGroups(),
                    deduplicationMapsByKeyGroup.size());
        ...
        LOG.info("Merge keyGroupID {} to deduplicationMapsByKeyGroup, current keyGroupRange {} with length: {}",
                mergedKeyGroup,
                keyGroupRange,
                deduplicationMapsByKeyGroup.size());
    }
