...
public class HeapKeyedStateBackend<K> extends AbstractKeyedStateBackend<K> {
    ...
    public void collectKeyedState(
        ...
        if(!registeredKVStates.isEmpty()){
            final FlexibleKeyGroupRange outgoingKeyGroupRange = (FlexibleKeyGroupRange)
                    FlexibleKeyGroupRange.fromIntegerList(keyGroups);
            collectRegisteredKVStatesInternal(outgoingKeyGroupRange, migratingState);
        }
    }

    @Override
    public void mergeKeyedState(StateBuffer sb){
        String stateName = sb.stateName;
        registeredKVStates.get(stateName).mergeStateTable(sb.outgoingManagedKeyedState);
    }

    private void collectRegisteredKVStatesInternal(
        ...
        migratingState.stateName = stateName;

        registeredKVStates.get(stateName).getStateBuffer(outgoingKeyGroupRange,migratingState);
    }
    ...
    public StateMap createStateMap(String stateName) {
        return registeredKVStates.get(stateName).createStateMap();
    }
    @Override
