...
public final class PrioritizedDeque<T> implements Iterable<T> {
    ...
    // Scale Utils
    // ------------------------------------------------------------------------
    public T peekNonPriorityWithCondition(Predicate<T> preCondition) {
        if (numPriorityElements == size() || deque.isEmpty()) {
            return null;
        }
        Iterator<T> iterator = deque.iterator();
        for (int i = 0; iterator.hasNext(); i++) {
            T next = iterator.next();
            if (i >= numPriorityElements && preCondition.test(next)) {
                return next;
            }
        }
        return null;
    }
    public T peekSecondNonPriorityWithCondition(Predicate<T> preCondition) {
        if (numPriorityElements == size() || deque.isEmpty()) {
            return null;
        }
        Iterator<T> iterator = deque.iterator();
    ...
    public int countNonPriorityWithCondition(Predicate<T> preCondition) {
        if (numPriorityElements == size() || deque.isEmpty()) {
            return 0;
        }
        int count = 0;
    ...
    public T peekLastNonPriorityWithCondition(Predicate<T> preCondition) {
        if (numPriorityElements == size() || deque.isEmpty()) {
            return null;
        }
        int size = deque.size();
        ...
        for (int i = 0; iterator.hasNext(); i++) {
            T next = iterator.next();
            int indexFromStart = size - 1 - i;
            if (indexFromStart >= numPriorityElements && preCondition.test(next)) {
                return next;
            }
        }
        return null;
    ...
    public T pollLastNonPriorityWithCondition(Predicate<T> preCondition) {
        if (numPriorityElements == size() || deque.isEmpty()) {
            return null;
        }
        int size = deque.size();
