...
public class ConsumedPartitionGroup implements Iterable<IntermediateResultPartitionID> {
    ...
    private List<IntermediateResultPartitionID> resultPartitions;

    private AtomicInteger unfinishedPartitions;
    ...
    public IntermediateResultPartitionID getLast() { return resultPartitions.get(resultPartitions.size()-1); }
    ...
    public String toString() {
        return "ConsumedPartitionGroup["
                + intermediateDataSetID+" "
                + resultPartitions.size() + " partitions("
                + unfinishedPartitions + " unfinished), "
                + numConsumers + " consumers"
                +"]";
    }

    public void addPartition(IntermediateResultPartitionID resultPartitionId) {
        checkArgument(unfinishedPartitions.get() == resultPartitions.size(),
        "There has been some partitions finished before scaling.");
        resultPartitions.add(resultPartitionId);
        this.unfinishedPartitions = new AtomicInteger(resultPartitions.size());
    }
}
