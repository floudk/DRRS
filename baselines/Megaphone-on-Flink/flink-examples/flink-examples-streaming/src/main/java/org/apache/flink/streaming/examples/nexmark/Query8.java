...
public class Query8 {
    public void run(NexmarkCLI cli) throws Exception {
        ...
        joinedStream.keyBy(tuple -> tuple.f0).filter(tuple -> RandomUtils.nextInt(0, 200) == 0)
                .sinkTo(localFileSink).name("FileSink").setParallelism(cli.specificParallelism).slotSharingGroup("file-sink");

//        KeyedStream<Tuple2<Long, Long>, Long> keyedSellerStream =
//                joinedStream.keyBy(tuple -> tuple.f0);
//        keyedSellerStream.sinkTo(localFileSink).name("FileSink").setParallelism(cli.specificParallelism).slotSharingGroup("file-sink");
