...
public abstract class AbstractStreamOperator<OUT>
    ...
    public FlexibleKeyGroupRange getFlexibleKeyGroupRange() {
        FlexibleKeyGroupRange flexibleKeyGroupRange = stateHandler.getFlexibleKeyGroupRange();
        if (timeServiceManager != null) {
            timeServiceManager.updateKeyGroupRange(flexibleKeyGroupRange);
        }
        return flexibleKeyGroupRange;
    }


    @Override
    public Optional<Integer> getKeyGroupByKey1(StreamRecord record) throws Exception {
        return getKeyGroupByKey(record, stateKeySelector1);
    }

    private <T> Optional<Integer> getKeyGroupByKey(StreamRecord<T> record, KeySelector<T, ?> selector)
            throws Exception {
        if (selector != null) {
            Object key = selector.getKey(record.getValue());
            return stateHandler.getKeyGroupByKey(key);
        }
        return Optional.empty();
    }

    @Override
    public StateBuffer collectState(List<Integer> keyGroups) throws IOException {
        return stateHandler.collectState(keyGroups,Optional.ofNullable(timeServiceManager));
    }

    @Override
    public void mergeState(StateBuffer stateBuffer){
        stateHandler.mergeState(stateBuffer);
        if (timeServiceManager != null) {
    ...
    public StateMap createStateMap(String stateName){
        return stateHandler.createStateMap(stateName);
    }
