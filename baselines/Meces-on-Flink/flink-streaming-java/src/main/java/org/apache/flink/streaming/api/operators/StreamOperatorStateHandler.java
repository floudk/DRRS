...
public class StreamOperatorStateHandler {
    ...
    public ScalingContext.ContextInitInfo getScalingContextInitInfo() {
        if (keyedStateBackend == null) {
            ...
            return ((ScalableStateBackend) keyedStateBackend).getScalingContextInitInfo();
        } else {
            throw new IllegalStateException(
    ...
    public ScaleBuffer.StateBuffer collectState(
            KeyOrStateID keyOrStateID,
            Optional<InternalTimeServiceManager<?>> timeServiceManager,
            boolean clearTimeService
    )throws IOException {

        if (timeServiceManager.isPresent()&&clearTimeService){
            LOG.info("Collecting time service for state {}", keyOrStateID);
            final InternalTimeServiceManager<?> manager = timeServiceManager.get();
            manager.collectRawKeyedState(keyOrStateID, null,null);
        }
        ...
        if( null != keyedStateBackend){
            ...
            return ((ScalableStateBackend) keyedStateBackend).collectKeyedState(keyOrStateID);
        }
        return ScaleBuffer.StateBuffer.EMPTY_STATE_BUFFER;
    }

    public BitSet mergeState(ScaleBuffer.StateBuffer stateBuffer){
        return ((ScalableStateBackend) keyedStateBackend).mergeKeyedState(stateBuffer);
    }

    public StateMap createStateMap(String stateName, @Nullable BitSet initialBinFlags) {
        return ((ScalableStateBackend) keyedStateBackend).createStateMap(stateName,initialBinFlags);
    }
