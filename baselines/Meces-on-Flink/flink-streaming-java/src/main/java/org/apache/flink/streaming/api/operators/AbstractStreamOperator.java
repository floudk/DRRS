...
import org.apache.flink.runtime.state.heap.HierarchicalCopyOnWriteStateMap;
...
public abstract class AbstractStreamOperator<OUT>
    ...
    SubtaskScaleHandler scaleHandler;

    @Override
    public void setSubTaskScaleHandler(SubtaskScaleHandler scaleHandler){
        this.scaleHandler = scaleHandler;
    }

    @Override
    public ScalingContext.ContextInitInfo getScalingContextInitInfo() {
        ScalingContext.ContextInitInfo info = stateHandler.getScalingContextInitInfo();
        if (timeServiceManager !=null){
            timeServiceManager.updateKeyGroupRange(info.currentKeyGroups);
        }
        return info;
    }

    @Override
    public Tuple2<Optional<Integer>, Optional<Integer>> tryGetHieraGroupIndexByRecord(StreamRecord record) throws Exception {
        return tryGetHieraGroupIndexByRecord(record, stateKeySelector1);
    }
    @Override
    public HierarchicalStateID getHieraStateIDByKey(Object key) {
        int keyGroupIndex = stateHandler.getKeyGroupByKey(key).orElseThrow(() -> new RuntimeException("keyGroupIndex is null"));
        int subBinGroupIndex = HierarchicalCopyOnWriteStateMap.reHash(key);
        return new HierarchicalStateID(keyGroupIndex, subBinGroupIndex);
    }

    private <T> Tuple2<Optional<Integer>, Optional<Integer>> tryGetHieraGroupIndexByRecord(StreamRecord<T> record, KeySelector<T, ?> selector)
            throws Exception {
        if (selector != null) {
            Object key = selector.getKey(record.getValue());
            int subBinGroupIndex = HierarchicalCopyOnWriteStateMap.reHash(key);
            return Tuple2.of(stateHandler.getKeyGroupByKey(key), Optional.of(subBinGroupIndex));
        }
        return Tuple2.of(Optional.empty(), Optional.empty());
    }

    @Override
    public ScaleBuffer.StateBuffer collectState(KeyOrStateID keyOrStateID,boolean clearTimeService) throws IOException {
        return stateHandler.collectState(keyOrStateID, Optional.ofNullable(timeServiceManager), clearTimeService);
    }

    @Override
    public BitSet mergeState(ScaleBuffer.StateBuffer stateBuffer){
        BitSet res = stateHandler.mergeState(stateBuffer);
        if (timeServiceManager != null){
            timeServiceManager.updateTimeServiceForStateMerge(stateBuffer);
        }else{
            LOG.info("timeServiceManager is null when mergeState");
        }
        return res;
    }

    @Override
    public StateMap createStateMap(String stateName, @Nullable BitSet initialBinFlags){
        return stateHandler.createStateMap(stateName, initialBinFlags);
    }

    @Override
    public Tuple3<TypeSerializer,TypeSerializer,TypeSerializer> getSerializers(String stateName){
        return stateHandler.getSerializers(stateName);
    }
    ...
    public boolean trySetCurrentKey(Object key) throws Exception {
        if(key == null){
            scaleHandler.trySetCurrentKey(null);
            return false;
        }
        if (scaleHandler == null || !scaleHandler.isScaling()){
            setCurrentKey(key);
            return true;
        }else{
            try {
                if (scaleHandler.trySetCurrentKey(key)){
                    setCurrentKey(key);
                    return true;
                }
            } catch (Exception e) {
                LOG.error("trySetCurrentKey error", e);
