...
public class RecordProcessorUtils {
    ...
    public static <T> ThrowingBiConsumer<StreamRecord<T>, InputChannelInfo, Exception> getScalingRecordProcessor(
            Input<T> input, SubtaskScaleHandler subscaleHandler, Counter numRecordsIn) {

        checkNotNull(subscaleHandler, "subscaleHandler is null");
        ...
        if (canOmitSetKeyContext) {
            return (record, unused) -> input.processElement(record);
        } else {
            return (record, channelInfo) -> {
                Tuple2<Optional<Integer>, Optional<Integer>> groupIDs = input.getKeyGroupByKey(record);
                if(groupIDs.f0.isEmpty() || groupIDs.f1.isEmpty()) {
                    throw new RuntimeException(
                            "KeyGroup is empty, please check the key selector and key group assignment");
                }
                int keyGroupIndex = groupIDs.f0.get();
                int subBinGroupIndex = groupIDs.f1.get();

                subscaleHandler.processRecordOnScaling(
                        record,
                        channelInfo,
                        keyGroupIndex,
                        subBinGroupIndex,
