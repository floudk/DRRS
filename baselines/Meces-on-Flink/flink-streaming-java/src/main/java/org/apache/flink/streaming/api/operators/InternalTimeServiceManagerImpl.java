...
public class InternalTimeServiceManagerImpl<K> implements InternalTimeServiceManager<K> {
    ...
    @Override
    public void collectRawKeyedState(
            KeyOrStateID keyOrStateID,
            ScaleBuffer.StateBuffer migratingState,
            ByteBuf byteBuf){

        int keyGroup = keyOrStateID.getKey();
        final Map<String, InternalTimerServiceImpl<K, ?>> registeredTimerServices =
                getRegisteredTimerServices();
        try{
            // Map<Integer, ScaleBuffer.StateBuffer.TimerServiceState> timerServiceStateMap = new HashMap<>();
//            ByteBufDataOutputView byteBufDataOutputView = new ByteBufDataOutputView(byteBuf);
            for (Map.Entry<String, InternalTimerServiceImpl<K, ?>> entry : registeredTimerServices.entrySet()) {
                ...
                checkState(
                        localKeyGroupRange.contains(keyGroup),
                        "Key Group " + keyGroup + " does not belong to the local range.");
                timerService.collectTimersForKeyGroup(keyGroup);
...
//                                    .writeTimersSnapshot(byteBufDataOutputView);
    ...
    @Override
    public void updateTimeServiceForStateMerge(ScaleBuffer.StateBuffer stateBuffer){
        Map<KeyOrStateID, StateMap> mergedStates= stateBuffer.outgoingManagedKeyedState;
        KeyOrStateID mergeOrFetchedState = mergedStates.keySet().iterator().next();
        for (InternalTimerServiceImpl<K, ?> timerService : timerServices.values()) {
            timerService.updateTimeServiceForStateMerge(mergeOrFetchedState);
        }
    }
}
