...
public final class StreamElementSerializer<T> extends TypeSerializer<StreamElement> {
    ...
    public void copy(DataInputView source, DataOutputView target) throws IOException {
        ...
        if (tag == TAG_REC_WITH_TIMESTAMP) {
            ...
            boolean customized = source.readBoolean();
            if (customized){
                target.writeLong(source.readLong());
                target.writeLong(source.readLong());
            }else{
    ...
    public void serialize(StreamElement value, DataOutputView target) throws IOException {
        if (value.isRecord()) {
            ...
            LatencyMarker marker = value.asLatencyMarker();
            target.writeBoolean(marker.customized);
            if (marker.customized){
                target.writeLong(marker.getMarkedTime());
                String operatorName = marker.emitOperatorName;
                target.writeUTF(operatorName);
                target.writeLong(marker.outputTime);
            }else{
    ...
    public StreamElement deserialize(DataInputView source) throws IOException {
        ...
        if (tag == TAG_REC_WITH_TIMESTAMP) {
            ...
            boolean customized = source.readBoolean();
            if (customized){
                return new LatencyMarker(source.readLong(), source.readUTF(), source.readLong());
            }
