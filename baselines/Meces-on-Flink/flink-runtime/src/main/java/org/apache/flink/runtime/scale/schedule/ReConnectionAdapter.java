...
public class ReConnectionAdapter {
    ...
    static Logger LOG = LoggerFactory.getLogger(ReConnectionAdapter.class);
    ...
    private void clearCachedInformationForPartitionGroup(
        ...
        if(result.getCachedShuffleDescriptors(currentConsumedPartitionGroup) != null) {
            ...
            for (ConsumedPartitionGroup cacheKey : cacheKeys) {
                LOG.info("Clearing cached shuffle descriptors for partition group {} of result {}.",
                        cacheKey, resultID);
                result.clearCachedInformationForPartitionGroup(cacheKey);
            }
