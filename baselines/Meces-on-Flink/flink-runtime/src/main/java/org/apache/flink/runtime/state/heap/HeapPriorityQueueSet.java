...
public class HeapPriorityQueueSet<T extends HeapPriorityQueueElement> extends HeapPriorityQueue<T>
    ...
    private int globalKeyGroupToLocalIndex(int keyGroup) {
        ...
        if (offset >= deduplicationMapsByKeyGroup.size()) {
            LOG.warn(
                    "Key group {} (index {}) is not in the expected range {}(length: {}), while deduplicationMapsByKeyGroup has length {}",
                    keyGroup,
                    offset,
                    keyGroupRange,
    ...
    public Set<T> getAndRemoveSubsetForKeyGroup(int keyGroupId) {
        ...
        // remove elements from dedupMap and super
        for (T element : subset) {
            try {
                super.remove(element);
            } catch (NullPointerException e) {
                // there may be NullPointerException when there is no any record come after merge to register any trigger,
                // we just ignore it
                LOG.warn("NullPointerException due to no any record come after merge to register any trigger, ignore it");
            }
        }
        dedupMap.clear();
...
//        LOG.info("Remove keyGroupID {} from deduplicationMapsByKeyGroup, current keyGroupRange {} with length: {}",
//                keyGroupId,
//                keyGroupRange,
//                deduplicationMapsByKeyGroup.size());
        return subset;
    ...
    @Override
    public void mergeKeyGroup(KeyOrStateID mergedKeyGroup){
        if (keyGroupRange.getNumberOfKeyGroups() <= deduplicationMapsByKeyGroup.size()){
            return;
        }
        int offset = keyGroupRange.keyGroupIndexToOffset(mergedKeyGroup.getKey());
        ...
        if (keyGroupRange.getNumberOfKeyGroups() != deduplicationMapsByKeyGroup.size()){
            LOG.error("Merge keyGroupID {}(offset {}) to deduplicationMapsByKeyGroup, current keyGroupRange {}({}) with deduplicationMapsByKeyGroup length: {}",
                    mergedKeyGroup,
                    offset,
                    keyGroupRange,
                    keyGroupRange.getNumberOfKeyGroups(),
                    deduplicationMapsByKeyGroup.size());
            throw new IllegalStateException("Merge keyGroupID to deduplicationMapsByKeyGroup reach unexpected state");
        }

//        LOG.info("Merge keyGroupID {} to deduplicationMapsByKeyGroup, current keyGroupRange {}({}) with deduplicationMapsByKeyGroup length: {}",
//                mergedKeyGroup,
//                keyGroupRange,
//                keyGroupRange.getNumberOfKeyGroups(),
//                deduplicationMapsByKeyGroup.size());
