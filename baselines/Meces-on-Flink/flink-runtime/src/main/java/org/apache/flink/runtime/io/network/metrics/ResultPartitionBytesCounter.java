...
public class ResultPartitionBytesCounter {
    ...
    public void updateCounterNumSubpartitions(int numSubpartitions) {
        if (numSubpartitions > subpartitionBytes.size()) {
            // scale out
            for (int i = subpartitionBytes.size(); i < numSubpartitions; ++i) {
                subpartitionBytes.add(new SimpleCounter());
            }
        } else if (numSubpartitions < subpartitionBytes.size()) {
            // scale in
            throw new UnsupportedOperationException(
                    "Scale in is not supported for ResultPartitionBytesCounter.");
        }
    }
}
