...
public class ScaleCommListener {
    ...
    private BiConsumer<ScaleEvent,Channel> eventConsumer;

    public void setConsumers(
            BiConsumer<ScaleBuffer, Integer>  bufferConsumer, BiConsumer<ScaleEvent, Channel> eventConsumer) {
    ...
    public void onEvent(ScaleEvent event, Channel channel) {
        checkNotNull(eventConsumer, "Event consumer is not set");
        eventConsumer.accept(event, channel);
    }
}
