...
import static org.apache.flink.runtime.state.heap.HierarchicalCopyOnWriteStateMap.reHash;
...
public class HeapKeyedStateBackend<K> extends AbstractKeyedStateBackend<K> {
    ...
    private <N, V> StateTable<K, N, V> tryRegisterStateTable(
        ...
        if (stateTable != null) {
            ...
            stateTable = stateTableFactory.newStateTable(
                    keyContext, newMetaInfo, keySerializer, stateDesc.isEnableHierarchical());
    ...
    public ScalingContext.ContextInitInfo getScalingContextInitInfo(){
        if(registeredKVStates.keySet().size() > 1){
            LOG.warn("Not support multiple stateName now");
            throw new UnsupportedOperationException("Not support multiple stateName now");
        }
        // get the only key
        String stateName = registeredKVStates.keySet().iterator().next();
        StateTable stateTable = registeredKVStates.get(stateName);
        return stateTable.getScalingContextInitInfo();
    }


    @Override
    public ScaleBuffer.StateBuffer collectKeyedState(KeyOrStateID keyOrStateID) {

        if(registeredKVStates.isEmpty() && priorityQueuesManager.getRegisteredPQStates().isEmpty()){
            LOG.warn("No state to collect in HeapKeyedStateBackend");
            return ScaleBuffer.StateBuffer.EMPTY_STATE_BUFFER;
        }

        if(!registeredKVStates.isEmpty()){
            return collectRegisteredKVStatesInternal(keyOrStateID);
        }

        if(!priorityQueuesManager.getRegisteredPQStates().isEmpty()){
            LOG.info("Try to collect keyed state for priorityQueuesManager");
            LOG.error("To be implemented");
            throw new UnsupportedOperationException("To be implemented");
        }
        throw new UnsupportedOperationException("Unexpected return path in collectKeyedState, need to further implement");
    }

    @Override
    public BitSet mergeKeyedState(ScaleBuffer.StateBuffer sb){
        String stateName = sb.stateName;
        return registeredKVStates.get(stateName).mergeStateTable(sb.outgoingManagedKeyedState);
    }

    private ScaleBuffer.StateBuffer collectRegisteredKVStatesInternal(KeyOrStateID keyOrStateID) {
        if(registeredKVStates.keySet().size() > 1){
            LOG.warn("Not support multiple stateName now");
            throw new UnsupportedOperationException("Not support multiple stateName now");
        }
        ...
        String stateName = registeredKVStates.keySet().iterator().next();
        StateTable stateTable = registeredKVStates.get(stateName);
        return stateTable.getStateBuffer(keyOrStateID, stateName);
    }
    
    @Override
    public StateMap createStateMap(String stateName, @Nullable BitSet initialBinFlags) {
        return registeredKVStates.get(stateName).createStateMap(initialBinFlags);
    }
    @Override
    public Tuple3<TypeSerializer,TypeSerializer,TypeSerializer> getSerializers(String stateName){
        return registeredKVStates.get(stateName).getSerializers();
    }
    @Override
    public StateSnapshotTransformer getStateSnapshotTransformer(String stateName){
        return registeredKVStates.get(stateName).getStateSnapshotTransformer();
    }

    @Override
    public FlexibleKeyGroupRange getFlexibleKeyGroupRange() {
        throw new IllegalStateException("Not support getFlexibleKeyGroupRange in HeapKeyedStateBackend");
    }
}
