...
class HeapMetaInfoRestoreOperation<K> {
    private static final Logger LOG = LoggerFactory.getLogger(HeapMetaInfoRestoreOperation.class);
                    ...
                    if (registeredState == null) {
                        RegisteredKeyValueStateBackendMetaInfo<?, ?>
                                registeredKeyedBackendStateMetaInfo =
                                        new RegisteredKeyValueStateBackendMetaInfo<>(
                                                metaInfoSnapshot);
                        // temporarily hard code enableHierarchical to false
                        // since we may not need to restore hierarchical state
                        // however, this should be checked again in LOGs to make sure
                        LOG.warn("Creating new state table for {} with not enableHierarchical", metaInfoSnapshot.getName());
                        ...
                        registeredKVStates.put(
                                ...
                                stateTableFactory.newStateTable(
                                        keyContext,
                                        registeredKeyedBackendStateMetaInfo,
                                        keySerializerProvider.currentSchemaSerializer(),false));
                    }
                    break;
                case PRIORITY_QUEUE:
