...
public class InputGateWithMetrics extends IndexedInputGate {
    ...
    public Optional<SingleInputGate> getConsumedResultId() {
        if(inputGate instanceof SingleInputGate) {
          return Optional.of((SingleInputGate) inputGate);
        } else {
    ...
    public String getOwnerName() {
        if(inputGate instanceof SingleInputGate) {
            return ((SingleInputGate) inputGate).getOwningTaskName();
        } else {
            return inputGate.toString();
        }
    }

    @Override
    public boolean isUpdatingForUpstreamScaling(){
        return inputGate.isUpdatingForUpstreamScaling();
    }

    public void unsetUpdatingForUpstreamScaling(){
        ((SingleInputGate) inputGate).isUpdatingForUpstreamScaling = false;
        ((SingleInputGate) inputGate).addedChannelNumber = -1;
    }

    public int getNewlyAddedChannelNumber(){
        return ((SingleInputGate) inputGate).addedChannelNumber;
    }

    public Map<SingleInputGate.SubpartitionInfo, InputChannel> getInputChannels() {
        return ((SingleInputGate) inputGate).getInputChannels();
    }
