...
public class CopyOnWriteStateTable<K, N, S> extends StateTable<K, N, S> {
    ...
    CopyOnWriteStateTable(
        ...
        super(keyContext, metaInfo, keySerializer, enableHierarchicalState);
    }
    ...
    public CopyOnWriteStateMap<K, N, S> createStateMap() {
        if (enableHierarchicalState){
            return new HierarchicalCopyOnWriteStateMap<>(getStateSerializer());
        }
    ...
    // ---------------------------------- Scale Utils ----------------------------------
    @Override
    public CopyOnWriteStateMap<K, N, S> createStateMap(@Nullable BitSet initialBinFlags) {
        if (initialBinFlags!=null){
            checkState(enableHierarchicalState, "Hierarchical state is not enabled.");
            return new HierarchicalCopyOnWriteStateMap<>(getStateSerializer(),initialBinFlags);
        }
        return new CopyOnWriteStateMap<>(getStateSerializer());
    }
}
