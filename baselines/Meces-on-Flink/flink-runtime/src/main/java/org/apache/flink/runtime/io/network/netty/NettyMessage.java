...
public abstract class NettyMessage {
    protected static final Logger LOG = LoggerFactory.getLogger(NettyMessage.class);
    // ------------------------------------------------------------------------
    ...
    static public class CloseRequest extends NettyMessage {
    ...
    protected void handleException(@Nullable ByteBuf byteBuf, @Nullable Buffer buffer, Throwable t)
            throws IOException {
        if (byteBuf != null) {
            byteBuf.release();
        }
