...
public class RepartitionBuffersWithPartialRecord {
    ...
    @Nullable private MemorySegment partialRecordMemorySegment;
    ...
    public RepartitionBuffersWithPartialRecord(
        ...
        this.partialRecordMemorySegment = partialRecordMemorySegment;

        // initial the spanningHelper if needed
        if (partialRecordMemorySegment != null) {
            ...
            // only set the next buffer if there is no partial record
            setNextBuffer();
        }else{
            LOG.info("No records to read in the buffer");
        }
    }

    @Override
    public String toString() {
        return "Repartition buffers: " + buffers.size() + ", partial record length: " +
                (partialRecordMemorySegment == null ? 0 : partialRecordMemorySegment.size());
    }
    ...
    public <T extends IOReadableWritable> DeserializationResult readNextRecord(T target)
        ...
        if(result.isBufferConsumed()){
            Preconditions.checkNotNull(currentBuffer, "Current buffer should not be null");
    ...
    private static class SpanningHelper{
        ...
        public void transferFromPartialMemorySegment(MemorySegment partialSegment){
            ...
            if(!lengthBuffer.hasRemaining()) {
                ...
                partialRecord = new byte[recordLength];
                int toCopy = min(recordLength, partialSegment.size() - Integer.BYTES);
