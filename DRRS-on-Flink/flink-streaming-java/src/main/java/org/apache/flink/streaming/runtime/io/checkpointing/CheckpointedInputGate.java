...
public class CheckpointedInputGate implements PullingAsyncDataInput<BufferOrEvent>, Closeable {
    ...
    private ScaleBarrierHandler scaleBarrierHandler;
    ...
    private Optional<BufferOrEvent> handleEvent(BufferOrEvent bufferOrEvent) throws IOException {
        ...
        if (eventClass == CheckpointBarrier.class) {
            ...
            barrierHandler.processBarrierAnnouncement(
                    ...
                    bufferOrEvent.getChannelInfo());
        }else if (eventClass == TriggerBarrier.class){
            TriggerBarrier barrier = (TriggerBarrier) bufferOrEvent.getEvent();
            scaleBarrierHandler.processScaleTriggerBarrier(barrier, bufferOrEvent.getChannelInfo());
        } else if (eventClass == ConfirmBarrier.class){
            ConfirmBarrier barrier = (ConfirmBarrier) bufferOrEvent.getEvent();
            scaleBarrierHandler.processScaleConfirmBarrier(barrier, bufferOrEvent.getChannelInfo());
        } else if (eventClass == ScaleBarrier.CompleteBarrier.class) {
            scaleBarrierHandler.processScaleCompleteBarrier();
        }
    ...
    public void setScaleHandler(ScaleBarrierHandler scaleBarrierHandler) {
        this.scaleBarrierHandler = scaleBarrierHandler;
    }


    private String getOwnerName() {
        return ((InputGateWithMetrics) inputGate).getOwnerName().split("#")[0];
    }
