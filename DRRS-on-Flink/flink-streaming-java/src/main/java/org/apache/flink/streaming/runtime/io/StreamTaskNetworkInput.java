...
public final class StreamTaskNetworkInput<T>
    ...
    // --------------------------------------------------------------------------------------------
    @Override
    public void expandRecordDeserializers(
            int newChannelNum,
            List<InputChannelInfo> addedChannelInfos,
            IOManager ioManager){
        // add new channels
        addedChannelInfos.forEach(
                channelInfo -> {
                    this.recordDeserializers.put(
                            channelInfo,
                            new SpillingAdaptiveSpanningRecordDeserializer<>(
                                    ioManager.getSpillingDirectoriesPaths()));
                });

        // update flattenedChannelIndices
        flattenedChannelIndices.clear();
        for(InputChannelInfo channelInfo : checkpointedInputGate.getChannelInfos()){
            flattenedChannelIndices.put(channelInfo, flattenedChannelIndices.size());
        }

        // update statusWatermarkValve
        statusWatermarkValve.expandChannelStatuses(newChannelNum);
    }
}
