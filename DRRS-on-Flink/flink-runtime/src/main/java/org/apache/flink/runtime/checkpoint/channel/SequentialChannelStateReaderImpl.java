...
public class SequentialChannelStateReaderImpl implements SequentialChannelStateReader {
    ...
    @Override
    public void readInputData(Collection<RecoveredInputChannel> channels) throws Exception {
        // do something about recover input channel state
        try (InputChannelScalingRecoveredStateHandler stateHandler =
                     new InputChannelScalingRecoveredStateHandler(
                             channels, taskStateSnapshot.getInputRescalingDescriptor()
                     )){
            // TODO: do something about recover input channel state
