...
class InputChannelScalingRecoveredStateHandler
        implements RecoveredChannelStateHandler<InputChannelInfo, Buffer> {

    private final Collection<RecoveredInputChannel> inputChannels;
    private final InflightDataRescalingDescriptor channelMapping;


    InputChannelScalingRecoveredStateHandler(
            Collection<RecoveredInputChannel> inputChannels, InflightDataRescalingDescriptor channelMapping) {
    ...
    @Override
    public BufferWithContext<Buffer> getBuffer(InputChannelInfo inputChannelInfo) throws IOException, InterruptedException {
        return null;
    }
    ...
    public void recover(
            ...
            int oldSubtaskIndex,
            BufferWithContext<Buffer> bufferWithContext) throws IOException {

    }

    @Override
    public void close() throws Exception {
        for (final RecoveredInputChannel inputChannel : inputChannels) {
            inputChannel.finishReadRecoveredState();
        }
    }
}
