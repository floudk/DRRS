...
public interface JobMasterGateway
    ...
    // ----------------------------- scale utils -----------------------------
    default CompletableFuture<Void> triggerScale(
            TriggerId triggerID,
            String operatorName,
            int newParallelism,
    ...
    default CompletableFuture<Acknowledge> triggerSubscale(
            List<Integer> keys, TriggerId triggerID,  @RpcTimeout Time timeout){
        throw new UnsupportedOperationException("triggerSubscale is not supported in "
                + getClass().getSimpleName());
    }

    default CompletableFuture<Acknowledge> acknowledgeScaleComplete(ExecutionAttemptID executionAttemptID){
        throw new UnsupportedOperationException("notifyScaleComplete is not supported in "
                + getClass().getSimpleName());
    }
