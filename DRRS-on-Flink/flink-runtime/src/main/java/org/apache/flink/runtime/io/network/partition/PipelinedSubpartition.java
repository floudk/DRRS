...
public class PipelinedSubpartition extends ResultSubpartition
    ...
    public void addRecovered(BufferConsumer bufferConsumer) throws IOException {
        NetworkActionsLogger.traceRecover(
                ...
                bufferConsumer,
                parent.getOwningTaskName().split("#")[0],
                subpartitionInfo);
    ...
    public int getConfiguredBufferSize() {
        return receiverExclusiveBuffersPerChannel;
    }
    ...
    public void addConfirmBarrierAsSemiPriority(
        ...
        synchronized (buffers) {
            if (isFinished || isReleased) {
                bufferConsumer.close();
                LOG.error("{} Add buffer to finished or released subpartition {}",
                        parent.getOwningTaskName().split("\\(")[0],
                        getSubPartitionIndex());
                throw new IllegalStateException("Add buffer to finished or released subpartition");
            }
            ...
            LOG.info(" Add semi-priority buffer to subpartition {} with nonP/nonPWithAllPartial/total buffers: {}/{}/{}, with buffersInBacklog: {}",
                    getSubPartitionIndex(),
                    nonPriorityBufferCount,
                    nonPriorityBufferCountWithAllPartialRecord,
                    buffers.size(),
                    buffersInBacklog);

            if (nonPriorityBufferCountWithAllPartialRecord > 0 && nonPriorityBufferCount > 1) {
                BufferConsumerWithPartialRecordLength fistNotAllPartial =
                        buffers.peekNonPriorityWithCondition(isNonAllPartialBuffer);
                checkNotNull(fistNotAllPartial, "The first buffer with partial record length must not be null");

                Stack<BufferConsumerWithPartialRecordLength> reDistributeStack = new Stack<>();
                while (buffers.peekLastNonPriorityWithCondition(isBuffer) != fistNotAllPartial) {
                    reDistributeStack.push(buffers.pollLastNonPriorityWithCondition(isBuffer));
                }
                LOG.info("fistNotAllPartial with partial record length: {} with reDistributeStack size: {} (nonP/nonPWithAllPartial/total: {}/{}/{})",
                        fistNotAllPartial.getPartialRecordLength(), reDistributeStack.size(),
                        buffers.countNonPriorityWithCondition(isBuffer),
                        buffers.countNonPriorityWithCondition(isNonAllPartialBuffer),
                        buffers.size());

                buffers.pollLastNonPriorityWithCondition(isBuffer);
                MemorySegment remainingPartialRecord  = null;
                buffersInBacklog = buffersInBacklog - reDistributeStack.size() - 1;
                totalNumberOfBuffers = totalNumberOfBuffers - reDistributeStack.size() - 1;

                if (fistNotAllPartial.getPartialRecordLength() == 0){
                    reDistributeStack.push(fistNotAllPartial);
                }else{
                    remainingPartialRecord = trimBuffer(fistNotAllPartial);
                    buffersInBacklog++;
                    totalNumberOfBuffers++;
                }
            ...
            BufferConsumerWithPartialRecordLength confirmBarrier =
                    new BufferConsumerWithPartialRecordLength(bufferConsumer, 0);
            buffers.add(confirmBarrier);
            totalNumberOfBuffers++;
    ...
    private MemorySegment trimBuffer(BufferConsumerWithPartialRecordLength firstBuffer){
        // trim the first buffer to ending without last partial record
        BufferConsumer bufferConsumer = firstBuffer.getBufferConsumer();
        ...
        int originReadIndex = buffer.getReaderIndex();
        int curReadIndex = originReadIndex + partialRecordLength;
        int remaining = buffer.getSize() - partialRecordLength;


        MemorySegment partialRecordSegment = null;
        if (remaining > 0){
            partialRecordSegment = MemorySegmentFactory.wrap(new byte[remaining]);
            memorySegment.copyTo(curReadIndex, partialRecordSegment, 0, remaining);
        }

        LOG.info("After trimmed {} with remaining {} ", partialRecordLength, remaining);

        // add the trimmed first buffer to the buffers again
        BufferConsumer trimmedBufferConsumer = new BufferConsumer(buffer, partialRecordLength);
        BufferConsumerWithPartialRecordLength trimmedBuffer = new BufferConsumerWithPartialRecordLength(trimmedBufferConsumer, partialRecordLength);
        buffers.add(trimmedBuffer);
