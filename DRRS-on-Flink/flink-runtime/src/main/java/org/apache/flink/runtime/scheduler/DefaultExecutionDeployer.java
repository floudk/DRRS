...
public class DefaultExecutionDeployer implements ExecutionDeployer {
    ...
    public void validateExecutionStates(final Collection<Execution> executionsToDeploy) {
    ...
    public void transitionToScheduled(final List<Execution> executionsToDeploy) {
    ...
    public List<ExecutionDeploymentHandle> createDeploymentHandles(
            final List<Execution> executionsToDeploy,
            final Map<ExecutionVertexID, ExecutionVertexVersion> requiredVersionByVertex,
            final List<ExecutionSlotAssignment> executionSlotAssignments) {
    ...
    public CompletableFuture<Void> assignAllResourcesAndRegisterProducedPartitions(
            final List<ExecutionDeploymentHandle> deploymentHandles) {
    ...
    public List<ExecutionDeploymentHandle> createDeploymentHandlesForScaling(
            final List<Execution> executionsToDeploy,
            final Map<ExecutionVertexID, ExecutionVertexVersion> requiredVersionByVertex){
        validateExecutionStates(executionsToDeploy);
        ...
        log.info("Allocating slots for {} tasks: {}.",
                executionsToDeploy.size(),
                executionsToDeploy.stream()
                        .map(Execution::getVertex)
                        .map(ExecutionVertex::getTaskNameWithSubtaskIndex).collect(Collectors.toList())
        );
        ...
        for (ExecutionSlotAssignment executionSlotAssignment : executionSlotAssignments) {
            if(executionSlotAssignment == null){
                log.error("Failed to allocate slot for task.");
            }
        }

        return createDeploymentHandles(
                executionsToDeploy, requiredVersionByVertex, executionSlotAssignments);
    }

    public void deployAllForScaling(final List<ExecutionDeploymentHandle> deploymentHandles) {
        for (final ExecutionDeploymentHandle deploymentHandle : deploymentHandles) {
            final CompletableFuture<LogicalSlot> slotAssigned =
                    deploymentHandle.getLogicalSlotFuture();
