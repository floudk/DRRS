...
class LocalBufferPool implements BufferPool {
    ...
    private int numberOfRequiredMemorySegments;
    ...
    public void update(
        ...
        // Need further check to ensure the function is secure in concurrent environment.
        ...
        checkArgument(
                ...
                maxNumberOfMemorySegments,
                numberOfRequiredMemorySegments);
        if(newNumberOfRequiredMemorySegments != this.numberOfRequiredMemorySegments
            || newMaxNumberOfMemorySegments != this.maxNumberOfMemorySegments) {
            this.numberOfRequiredMemorySegments = newNumberOfRequiredMemorySegments;
            this.maxNumberOfMemorySegments = newMaxNumberOfMemorySegments;
            this.currentPoolSize = Math.max(newNumberOfRequiredMemorySegments, currentPoolSize);
        }

        if (newNumberOfSubpartitions!= this.subpartitionBuffersCount.length && newNumberOfSubpartitions > 0) {
            if(this.subpartitionBuffersCount.length ==0){
                checkArgument(
                        maxBuffersPerChannel > 0,
                        "Maximum number of buffers for each channel (%s) should be larger than 0.",
                        maxBuffersPerChannel);
            ...
            if(newNumberOfSubpartitions<this.subpartitionBuffersCount.length) {
                throw new UnsupportedOperationException("Shrinking the number of subpartitions is not supported yet.");
            }

            // copy subpartitionBuffersCount to new array
            synchronized (availableMemorySegments) {
                int[] newSubpartitionBuffersCount = new int[newNumberOfSubpartitions];
                System.arraycopy(
                        subpartitionBuffersCount,
                        0,
                        newSubpartitionBuffersCount,
                        0,
                        Math.min(subpartitionBuffersCount.length, newNumberOfSubpartitions));
                this.subpartitionBuffersCount = newSubpartitionBuffersCount;
            }

            // copy subpartitionBufferRecyclers to new array
            BufferRecycler[] newSubpartitionBufferRecyclers = new BufferRecycler[newNumberOfSubpartitions];
            System.arraycopy(subpartitionBufferRecyclers, 0, newSubpartitionBufferRecyclers, 0, Math.min(subpartitionBufferRecyclers.length, newNumberOfSubpartitions));
            for (int i = subpartitionBufferRecyclers.length; i < newNumberOfSubpartitions; i++) {
                newSubpartitionBufferRecyclers[i] = new SubpartitionBufferRecycler(i, this);
            }
            this.subpartitionBufferRecyclers = newSubpartitionBufferRecyclers;
