...
public final class KeyGroupRangeAssignment {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(KeyGroupRangeAssignment.class);
    ...
    // Scale Utils
    // ------------------------------------------------------------------------
    public static int assignKeyToParallelOperatorWithRoutingTable(
            Object key,
            int maxParallelism,
            int[] routingTable){
        Preconditions.checkNotNull(key, "Assigned key must not be null!");
        int keyGroupId = computeKeyGroupForKeyHash(key.hashCode(), maxParallelism);

        int res = routingTable[keyGroupId];
        // if no such key group throw exception
        if (res == -1) {
            throw new IllegalArgumentException("No such key group: " + keyGroupId);
        }

//        LOG.info("Assign channel according to routing table: {} -> {}", keyGroupId, res);
        return res;
    }
}
