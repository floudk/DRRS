...
public class SingleInputGateFactory {
    ...
    public static int calculateNumChannels(
    ...
    //Scale Utils
    //----------------------------------------------------------------------------------------------
    public InputChannel[] getAddedInputChannels(
            SingleInputGate inputGate,
            DownstreamTaskDeployUpdateDescriptor descriptor){
        int numOriginalChannels = inputGate.getNumberOfInputChannels();
        int numAddedChannels = descriptor.getNumberOfNewChannels(numOriginalChannels);
        InputChannel[] addedChannels = new InputChannel[numAddedChannels];

        GateBuffersSpec gateBuffersSpec = getGateBuffersSpec(
                numOriginalChannels + numAddedChannels,
                inputGate.getConsumedPartitionType());

        int consumedSubpartitionIndex = descriptor.getConsumedSubpartitionIndex();

        for (int i = 0; i < numAddedChannels; i++) {
            int channelIndex = numOriginalChannels + i;
            addedChannels[i] = appendKnownInputChannel(
                    inputGate,
                    channelIndex,
                    gateBuffersSpec.getEffectiveExclusiveBuffersPerChannel(),
                    descriptor.getShuffleDescriptor(channelIndex),
                    consumedSubpartitionIndex,
                    inputGate.getInputChannelMetrics());
        }
    ...
    public InputChannel appendKnownInputChannel(
            SingleInputGate inputGate,
            int index,
            int buffersPerChannel,
            ...
            int consumedSubpartitionIndex,
            InputChannelMetrics metrics) {
        ResultPartitionID partitionId = shuffleDescriptor.getResultPartitionID();
        if (shuffleDescriptor.isLocalTo(taskExecutorResourceId)) {
            // Consuming task is deployed to the same TaskManager as the partition => local
            return new LocalRecoveredInputChannel(
                    inputGate,
                    index,
    ...
    public GateBuffersSpec getGateBuffersSpec(int numOfChannels, ResultPartitionType partitionType) {
        return createGateBuffersSpec(
                maxRequiredBuffersPerGate,
                configuredNetworkBuffersPerChannel,
                floatingNetworkBuffersPerGate,
                partitionType,
