...
public class EventSerializer {
    ...
    public static final byte SCALE_TRIGGER_EVENT = 21;
    public static final byte SCALE_CONFIRM_EVENT = 22;
    public static final byte SCALE_COMPLETE_EVENT = 23;
    ...
    public static ByteBuffer toSerializedEvent(AbstractEvent event) throws IOException {
        ...
        if (eventClass == EndOfPartitionEvent.class) {
            ...
            return ConfirmBarrier.serializeConfirmBarrier((ConfirmBarrier) event);
        } else if(eventClass == TriggerBarrier.class){
            return TriggerBarrier.serializeTriggerBarrier((TriggerBarrier) event);
        } else if(eventClass == ScaleBarrier.CompleteBarrier.class){
            ByteBuffer buf = ByteBuffer.allocate(4);
            buf.putInt(SCALE_COMPLETE_EVENT);
            buf.flip();
            return buf;
        } else {
            try {
    ...
    public static AbstractEvent fromSerializedEvent(ByteBuffer buffer, ClassLoader classLoader)
        ...
        try {
            ...
            } else if (type == SCALE_CONFIRM_EVENT) {
                return ConfirmBarrier.deserializeConfirmBarrier(buffer);
            } else if (type == SCALE_TRIGGER_EVENT) {
                return TriggerBarrier.deserializeTriggerBarrier(buffer);
            } else if (type == SCALE_COMPLETE_EVENT) {
                return new ScaleBarrier.CompleteBarrier();
