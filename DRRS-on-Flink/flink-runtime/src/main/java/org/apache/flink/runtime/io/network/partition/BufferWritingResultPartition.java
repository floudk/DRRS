...
public abstract class BufferWritingResultPartition extends ResultPartition {
    ...
    @Override
    public void emitTriggerBarrierEvent(TriggerBarrier triggerBarrier)throws IOException{
        checkInProduceState();
        ...
        for (Integer targetSubpartition : targetSubpartitions) {
            ...
            try (BufferConsumer eventBufferConsumer = ScaleBarrier.toBufferConsumer(
                    triggerBarrier, Buffer.DataType.TRIGGER_BARRIER)) {
    ...
    public Map<Integer,RepartitionBuffersWithPartialRecord> emitConfirmBarrierEvent(
        ...
        for(int i = 0; i < subpartitions.length; i++){
            if (!affectedSourceSubtasks.contains(i)) {
                // do not need to emit confirm barrier to unaffected subpartitions
                continue;
            }
            ResultSubpartition targetSubpartition = subpartitions[i];
