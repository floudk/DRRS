...
public class SingleInputGate extends IndexedInputGate {
     ...
     */
    private IndexRange subpartitionIndexRange;
    ...
    private BufferOrEvent transformEvent(
        ...
        if (event.getClass() == EndOfPartitionEvent.class) {
            ...
            synchronized (inputChannelsWithData) {
                ...
                if (getOwningTaskName().contains("Sink")){
                    LOG.info(
                            "{}: Received EndOfData from channel {}, {}/{}",
                            owningTaskName,
                            currentChannel.getChannelInfo().getInputChannelIdx(),
                            channelsWithEndOfUserRecords.cardinality(),
                            numberOfInputChannels);
                }
    ...
    public void addGateChannels(InputChannel[] newInputChannels) throws IOException{
        checkState(numberOfUninitializedChannels == 0,
                "There should not be uninitialized channels.");

        int oldLength = channels.length;

        InputChannel[] newChannels = new InputChannel[channels.length + newInputChannels.length];
        System.arraycopy(newInputChannels, 0, newChannels, channels.length, newInputChannels.length);

        synchronized (requestLock) {
            System.arraycopy(channels, 0, newChannels, 0, channels.length);
            this.channels = newChannels;
            for(int i = oldLength; i < newChannels.length; i++){
                InputChannel inputChannel = newChannels[i];
                IntermediateResultPartitionID partitionId =
                        inputChannel.getPartitionId().getPartitionId();
                int subpartitionIndex = inputChannel.getConsumedSubpartitionIndex();
                InputChannel res = inputChannels.put(
                        new SubpartitionInfo(partitionId, subpartitionIndex),
                        inputChannel);
                checkState(res == null,
                        "There should not be duplicated channels.");
                this.channels[i].setup();
            }
            this.numberOfInputChannels = this.channels.length;
        }

        int[] newLastPrioritySequenceNumber = new int[numberOfInputChannels];
        Arrays.fill(newLastPrioritySequenceNumber, oldLength,
                numberOfInputChannels, Integer.MIN_VALUE);
        BitSet newChannelsWithEndOfPartitionEvents = new BitSet(numberOfInputChannels);
        BitSet newChannelsWithEndOfUserRecords = new BitSet(numberOfInputChannels);
        synchronized (inputChannelsWithData) {
            System.arraycopy(lastPrioritySequenceNumber, 0,
                    newLastPrioritySequenceNumber, 0, lastPrioritySequenceNumber.length);
            this.lastPrioritySequenceNumber = newLastPrioritySequenceNumber;

            for(int i = 0; i < oldLength; i++){
                newChannelsWithEndOfPartitionEvents.set(i, channelsWithEndOfPartitionEvents.get(i));
                newChannelsWithEndOfUserRecords.set(i, channelsWithEndOfUserRecords.get(i));
            }
            this.channelsWithEndOfPartitionEvents = newChannelsWithEndOfPartitionEvents;
            this.channelsWithEndOfUserRecords = newChannelsWithEndOfUserRecords;
        }
    }


    public void requestPartitionsForUpstreamScaling(int oldChannelNum) {
        LOG.info("Requesting partitions for upstream scaling");
        this.isUpdatingForUpstreamScaling = true;
        this.addedChannelNumber = this.numberOfInputChannels - oldChannelNum;
        for(Map.Entry<SubpartitionInfo, InputChannel> entry : inputChannels.entrySet()){
            InputChannel inputChannel = entry.getValue();
            if(inputChannel.getChannelIndex()>=oldChannelNum){
                try {
                    checkState(inputChannel instanceof RecoveredInputChannel,
                            "Input channel should be RecoveredInputChannel");

                    ((RecoveredInputChannel) inputChannel).getStateConsumedFuture()
                            .thenRun(() -> {
                                try {
                                    LOG.info(" Input channel {} as Recovered Input Channel try to request subpartition",
                                    ...
                                    channels[inputChannel.getChannelIndex()] = realInputChannel;
                                    entry.setValue(realInputChannel);
                                    realInputChannel.requestSubpartition();
                                } catch (Exception e) {
                                    LOG.error("Fail to request subpartition", e);
    ...
    private InputChannelMetrics inputChannelMetrics = null; // only used for scaling
    public void setInputChannelMetrics(InputChannelMetrics inputChannelMetrics) {
        this.inputChannelMetrics = inputChannelMetrics;
    }
    public InputChannelMetrics getInputChannelMetrics(){
        return checkNotNull(inputChannelMetrics);
    }
    ...
    public boolean isUpdatingForUpstreamScaling = false;
    public int addedChannelNumber = -1;

    @Override
    public boolean isUpdatingForUpstreamScaling(){
        return isUpdatingForUpstreamScaling;
    }
