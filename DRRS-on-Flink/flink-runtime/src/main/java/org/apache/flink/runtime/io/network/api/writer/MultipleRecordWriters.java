...
public class MultipleRecordWriters<T extends IOReadableWritable>
    ...
    public RecordWriter getRecordWriterByPartitionID(ResultPartitionID resultPartitionID){
        List<RecordWriter<T>> matchedWriters = recordWriters.stream()
                .filter(writer -> writer.getTargetPartition().getPartitionId().equals(resultPartitionID))
                .collect(Collectors.toList());
        if(matchedWriters.size() == 1){
            return matchedWriters.get(0);
        }else{
            throw new IllegalArgumentException("Expect one matched writer, but got " + matchedWriters.size());
        }
    }
}
