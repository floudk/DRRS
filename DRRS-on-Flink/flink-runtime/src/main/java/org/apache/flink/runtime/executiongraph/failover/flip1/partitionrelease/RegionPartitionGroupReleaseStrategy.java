...
public class RegionPartitionGroupReleaseStrategy
    ...
    /**
     * Assume there is only one region, and all vertex in the region is not finished.
     */
    @Override
    public void expandSchedulingTopology(DefaultSchedulingPipelinedRegion pipelinedRegion,
                                         Boolean isScaledOut,
                                         List<SchedulingExecutionVertex> modifiedExecutionVertices,
                                         SlotAllocationAdapter slotAllocationStrategy) {


        List<ExecutionVertexID> modifiedExecutionVertexIds = modifiedExecutionVertices.stream()
                .map(SchedulingExecutionVertex::getId)
                .collect(Collectors.toList());
        // update regionExecutionViewByVertex
        if(isScaledOut){
            modifiedExecutionVertexIds.forEach(
                    id->regionExecutionViewByVertex.put(id,new PipelinedRegionExecutionView(pipelinedRegion)));
        }else {
            modifiedExecutionVertexIds.forEach(regionExecutionViewByVertex::remove);
        }

        // update partitionGroupConsumerRegions
        partitionGroupConsumerRegions.clear();
        Iterable<ConsumerRegionGroupExecutionView> newConsumerRegionGroups =
                initPartitionGroupConsumerRegions(schedulingTopology.getAllPipelinedRegions());


        // update consumerRegionGroupExecutionViewMaintainer
        consumerRegionGroupExecutionViewMaintainer.clear();
        consumerRegionGroupExecutionViewMaintainer.notifyNewRegionGroupExecutionViews(
                newConsumerRegionGroups);
