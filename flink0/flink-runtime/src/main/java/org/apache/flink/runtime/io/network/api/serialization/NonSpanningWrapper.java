...
public final class NonSpanningWrapper implements DataInputView {
    ...
    public void initializeFromMemorySegment(MemorySegment seg, int position, int limit) {
        this.segment = seg;
    ...
    public int copyContentTo(byte[] dst) {
    ...
    public void transferTo(ByteBuffer dst) {
        segment.get(position, dst, remaining());
        clear();
    }

    public DeserializationResult readInto(IOReadableWritable target) throws IOException {
        try {
            target.read(this);
    ...
    public boolean hasCompleteLength() {
        return remaining() >= LENGTH_BYTES;
    }

    public boolean canReadRecord(int recordLength) {
